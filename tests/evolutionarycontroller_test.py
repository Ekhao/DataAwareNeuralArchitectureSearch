...
class EvolutionaryontrollerTestCase(unittest.TestCase):
    ...
    def test_evaluate_fitness(self):
        ...
        input_model = inputmodel.InputModel(input_configuration=5, model_configuration=[9, 3, 5], search_space=searchspace.SearchSpace(([2,   4, 8, 16, 32, 64, 128], [
                                            3, 5], ["relu", "sigmoid"]), ([48000, 24000, 12000, 6000, 3000, 1500, 750, 325], ["spectrogram", "mel-spectrogram", "mfcc"])), dataset_loader=datasetloader.    DatasetLoader(constants.PATH_TO_NORMAL_FILES, constants.PATH_TO_ANOMALOUS_FILES, 9, 2, 1), frame_size=2048, hop_length=512, num_mel_banks=80, num_mfccs=13,     num_target_classes=2, model_optimizer=tf.keras.optimizers.Adam(),  model_loss_function=tf.keras.losses.SparseCategoricalCrossentropy(), model_metrics=["accuracy"],  model_width_dense_layer=10)
    ...
    def test_tournament_selection(self):
        ...
        dataset_loader = datasetloader.DatasetLoader(
            constants.PATH_TO_NORMAL_FILES, constants.PATH_TO_ANOMALOUS_FILES, 9, 2, 1)
        ...
        evolutionary_controller = evolutionarycontroller.EvolutionaryController(
            search_space=search_space, seed=32, trivial_initialization=True, population_size=2)
        input_configuration, model_configuration = evolutionary_controller.generate_configuration()
        input_model = inputmodel.InputModel(input_configuration, model_configuration, search_space, dataset_loader, 2048,
                                            512, 80, 13, 2, tf.keras.optimizers.Adam(), tf.keras.losses.SparseCategoricalCrossentropy(), ["accuracy"], 10)
