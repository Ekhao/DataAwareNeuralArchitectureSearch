...
class DatasetLoader:
    ...
    def load_dataset(self, path_normal_files, path_anomalous_files,sample_rate, preprocessing_type,
        ...
        normal_audio = Parallel(n_jobs=-1)(delayed(librosa.load)(file, sr=sample_rate) for file in normal_files)
        anomalous_audio = Parallel(n_jobs=-1)(delayed(librosa.load)(file, sr=sample_rate) for file in anomalous_files)

        match preprocessing_type:
            case "waveform":
                return (normal_audio, anomalous_audio)
            case "spectrogram":
                raise NotImplementedError("Loading data as a spectrogram has not been implemented yet")
            case "mel-spectrogram":
                raise NotImplementedError("Loading data as a mel-spectrogram has not been implemented yet")
            case "mfcc":
                raise NotImplementedError("Loading data as a MFCC has not been implemented yet")
