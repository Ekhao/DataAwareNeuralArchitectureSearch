...
class EvolutionaryController(controller.Controller):
    ...
    def __init__(self, search_space: SearchSpace, population_size: int, max_num_layers: int, population_update_ratio: float, crossover_ratio: float, approximate_model_size: int, seed: Optional[int] = None) -> None:
    ...
    def initialize_controller(self, trivial_initialization: bool = True) -> None:
        ...
        if trivial_initialization:
            self.unevaluated_configurations = [(tuple(random.choice(x) for x in self.search_space.data_granularity_search_space), [
                                               tuple(random.choice(x) for x in self.search_space.model_layer_search_space) for layer in range(1)]) for i in range(self.population_size)]
        # Another common way to generate an intial configuration for evolutionary algorithms is to generate random models from the search space.
            ...
            self.unevaluated_configurations = [(tuple(random.choice(x) for x in self.search_space.data_granularity_search_space), [tuple(random.choice(
                x) for x in self.search_space.model_layer_search_space) for layer in range(random.randint(1, self.max_num_layers))]) for i in range(self.population_size)]
    ...
    def _evaluate_fitness(data_model: DataModel, model_size_approximate_range: int) -> float:
    ...
    def _get_breeder_configurations(self, breeders: list[Individual]) -> list[Configuration]:
        return [(breeder[0].data_configuration,
                 breeder[0].model_configuration) for breeder in breeders]

    def _create_mutations(self, configurations_to_mutate: list[Configuration], amount: int) -> list[Configuration]:
    ...
    def _new_convolutional_layer_mutation(self, configuration: Configuration) -> Configuration:
        mutation = copy.deepcopy(configuration)
        new_conv_layer = tuple(random.choice(
            x) for x in self.search_space.model_layer_search_space)
    ...
    def _remove_convolutional_layer_mutation(self, configuration: Configuration) -> Configuration:
        mutation = copy.deepcopy(configuration)
        assert type(mutation[1]) == list
        if len(mutation[1]) > 1:
            mutation[1].pop()
    ...
    def _increase_filter_size_mutation(self, configuration: Configuration) -> Configuration:
        mutation = copy.deepcopy(configuration)
        assert type(mutation[1] == list)
        ...
        # Encode layer again
        layer_to_modify = (
            layer_to_modify[0], new_filter_size, layer_to_modify[2])
    ...
    def _decrease_filter_size_mutation(self, configuration: Configuration) -> Configuration:
        mutation = copy.deepcopy(configuration)
        assert type(mutation[1] == list)
        random_conv_layer_number = self._random_conv_layer_number(
            mutation)
        ...
        current_filter_size = layer_to_modify[1]
        new_filter_size = None
        for seach_space_filter_size in reversed(self.search_space.model_layer_search_space[1]):
            if seach_space_filter_size < current_filter_size:
                new_filter_size = seach_space_filter_size
                break
        if new_filter_size == None:
    ...
    def _increase_number_of_filters_mutation(self, configuration: Configuration) -> Configuration:
        mutation = copy.deepcopy(configuration)
        assert type(mutation[1] == list)
        random_conv_layer_number = self._random_conv_layer_number(
            mutation)
        ...
        # Encode layer again
        layer_to_modify = (new_filter_amount,
                           layer_to_modify[1], layer_to_modify[2])
    ...
    def _decrease_number_of_filters_mutation(self, configuration: Configuration) -> Configuration:
        mutation = copy.deepcopy(configuration)
        assert type(mutation[1] == list)
        random_conv_layer_number = self._random_conv_layer_number(
            mutation)
        ...
        current_filter_amount = layer_to_modify[0]
        new_filter_amount = None
        for seach_space_filter_amount in reversed(self.search_space.model_layer_search_space[0]):
            if seach_space_filter_amount < current_filter_amount:
                new_filter_amount = seach_space_filter_amount
                break
        if new_filter_amount == None:
        ...
        # Encode layer again
        layer_to_modify = (
            new_filter_amount, layer_to_modify[1], layer_to_modify[2])
    ...
    def _change_activation_function_mutation(self, configuration: Configuration) -> Configuration:
        mutation = copy.deepcopy(configuration)
        assert type(mutation[1] == list)
        random_conv_layer_number = self._random_conv_layer_number(
            mutation)
        ...
        # Encode layer again
        layer_to_modify = (
            layer_to_modify[0], layer_to_modify[1], new_activation_function)
    ...
    def _increase_sample_rate_mutation(self, configuration: Configuration) -> Configuration:
        ...
        for seach_space_sample_rate in reversed(self.search_space.data_granularity_search_space[0]):
    ...
    def _decrease_sample_rate_mutation(self, configuration: Configuration) -> Configuration:
        # Change sample rate. Sample rate in the current search space is in the first position of the search space tuple.
        current_sample_rate = configuration[0][0]
        new_sample_rate = None
        for seach_space_sample_rate in self.search_space.data_granularity_search_space[0]:
            if seach_space_sample_rate < current_sample_rate:
                new_sample_rate = seach_space_sample_rate
                break
        if new_sample_rate == None:
    ...
    def _change_preprocessing_mutation(self, configuration: Configuration) -> Configuration:
    ...
    def _create_crossovers(self, configurations_to_crossover: list[Configuration], amount: int) -> list[Configuration]:
        crossovers = []
        for i in range(amount):
    ...
    def _crossover(self, configuration1: Configuration, configuration2: Configuration) -> Configuration:
        new_data = (random.choice((configuration1[0][0], configuration2[0][0])), random.choice((
            configuration1[0][1], configuration2[0][1])))
        ...
        for i in range(min_layers):
            layer = (random.choice((configuration1[1][i][0], configuration2[1][i][0])), random.choice(
                (configuration1[1][i][1], configuration2[1][i][1])), random.choice((configuration1[1][i][2], configuration2[1][i][2])))
