...
class DatasetLoader:
    ...
    def supervised_dataset(
        self,
        normal_preprocessed: list[float],
        anomalous_preprocessed: list[float],
        test_size: float = 0.2,
    ...
    def load_dataset(
        ...
        frame_size: int,
        hop_length: int,
        num_mel_banks: Optional[int] = None,
        num_mfccs: Optional[int] = None,
    ) -> tuple[list, list]:
        ...
        normal_audio = Parallel(n_jobs=self.num_cores_to_use)(
            delayed(librosa.resample)(
                audio, orig_sr=self.base_sr, target_sr=target_sr, res_type="kaiser_fast"
            )
        ...
        match preprocessing_type:
            ...
            case "spectrogram":
                normal_spectogram = Parallel(n_jobs=self.num_cores_to_use)(
                    delayed(self._create_spectrogram)(audio, frame_size, hop_length)
                    for audio in normal_audio
                )
            ...
            case "mel-spectrogram":
                normal_mel_spectogram = Parallel(n_jobs=self.num_cores_to_use)(
                    delayed(self._create_mel_spectrogram)(
                        audio, target_sr, frame_size, hop_length, num_mel_banks
                    )
                    for audio in normal_audio
                )
